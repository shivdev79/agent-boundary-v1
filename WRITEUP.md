...
This means reward hacking is hard by design:
...
That's **3× better than random** and **1.8× better than heuristic** — the environment is learnable but not trivially solved.

### 2. LLM GRPO Training

Qwen2.5-0.5B-Instruct fine-tuned with TRL's GRPOTrainer using LoRA (r=16) on all attention and MLP projections. The reward signal comes directly from `grade_action()` — no reward model, no human labels. The model learns to produce valid JSON with the correct decision, justification, question focus, escalation target, tool selection, and audit note.
...
The only near-miss: on the payroll export (correct answer: REFUSE), it escalates instead — which still scores reasonably because the safety signal fires, but loses calibration points. This makes sense: the REINFORCE policy learned that escalating is almost always safe, so it over-applies it. This is exactly the kind of interpretable failure that makes the environment useful for studying agent behavior.
